  ...
  const handleScheduleNotification = async (scheduledData: any) => {
    try {
      const notification: Omit<
        ...
        "id" | "status" | "createdAt"
      > = {
        title: "🍻 Đi vụt thôiiii! 🏸",
        body: `📍 ${scheduledData?.courts?.name ?? ''}\n
              🗓️ ${formatDate(scheduledData?.date)}\n
              🕢 ${formatTime(scheduledData?.startTime)} - ${formatTime(scheduledData?.endTime)}`,
  ...
  const handleSaveSession = async (values: any) => {
    try {
      ...
      if (editingSession) {
        ...
        // Cập nhật giá trị cho scheduledTime
  ...
  const handleQrImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        setQrImage(data);
        // update formik field so formik.values.qrImage luôn sync
        try {
          formik.setFieldValue("qrImage", data);
        } catch (err) {
          // formik may not be ready in very early lifecycle — ignore safely
        }
      };
                ...
                <TextField
                  ...
                  helperText={
                    formik.touched.name && formik.errors.name
                      ? formik.errors.name
                      : "Tên tự động theo ngày/giờ. Có thể chỉnh sửa lại."
                  }
                  placeholder="VD: Thứ 5, 02/10/2025, 19:30-20:30"
                  InputProps={{
                    endAdornment: isAutoGeneratedSessionName(
                      formik.values.name
                    ) ? (
                      <Tooltip title="Tên được tạo tự động">
